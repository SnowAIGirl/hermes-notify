...
Sender name resolution:
  --from provided → role_map lookup, unmatched → use --from value as-is
  --from omitted  → no sender prefix (pure message)

Usage:
    python3 notify-agent.py --from <sender> <session> <message>
    python3 notify-agent.py --from <sender> --simple <session> <message>
    python3 notify-agent.py <session> <message>               # no sender prefix
"""
import os
import sys
import subprocess
...
def main():
    ...
    while args:
        if args[0] == '--config':
            ...
            break  # non-flag, stop parsing flags
